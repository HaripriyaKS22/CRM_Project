...
class _update_staffState extends State<update_staff> {
  ...
  // Map the family ID to its name
  String _getFamilyNameById(String familyId) {
    for (var family in fam) {
      if (family['id'] == familyId) {
        return family['name']; // Return the corresponding family name
      }
    }
    return 'Unknown'; // Return 'Unknown' if no family is found
  }
  ...
  TextEditingController familyy = TextEditingController();
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (widget.id == productsData['id']) {
          ...
          // Replace family ID with family name
          familyy.text = _getFamilyNameById(productsData['family']);
          print("Family Nameeeeeeeee: ${familyy.text}");
        }
        setState(() {
          _selectedFamily = [productsData['name'].toString()];
        });
      }
    } catch (error) {
      print("Error: $error");
...
void update(
  ...
  final token = await gettokenFromPrefs();
  try {
    var request = http.Request('PUT', Uri.parse('$api/api/staff/update/${widget.id}/'));
    ...
    // Prepare the JSON data for the request body with the correct formats
    Map<String, dynamic> data = {
      'date_of_birth': selectedDate.toIso8601String().substring(0, 10), // YYYY-MM-DD
      'driving_license_exp_date': selecteExp.toIso8601String().substring(0, 10), // YYYY-MM-DD
      'join_date': selectejoin.toIso8601String().substring(0, 10), // YYYY-MM-DD
      'confirmation_date': selecteconf.toIso8601String().substring(0, 10), // YYYY-MM-DD
      ...
      'supervisor_id': selectedmanagerId, // Ensure this is passed as an integer
      'gender': selectgender,
...
Future<void> updateImage(
    File? image1, File? image2, BuildContext scaffoldContext) async {
  final token = await gettokenFromPrefs();
  try {
    ...
    print("Image Upload Response: ${responseBody.body}");
    if (responseBody.statusCode == 200) {
      ScaffoldMessenger.of(scaffoldContext).showSnackBar(
        SnackBar(
          backgroundColor: Colors.green,
      ...
      ScaffoldMessenger.of(scaffoldContext).showSnackBar(
        SnackBar(content: Text('Image update failed. Please try again.')),
      );
    }
  } catch (e) {
    print("Error: $e");
                              ...
                              children: [
                                ...
                                fam.isEmpty
                                    ...
                                    : DropdownButton<String>(
                                        ...
                                        value: _selectedFamily.isEmpty
                                            ? null
                                            : _selectedFamily[0]
                                                .toString(), // Display selected family ID
                                        ...
                                        items: fam
                                          ...
                                          return DropdownMenuItem<String>(
                                            value: family[
                                                'id'], // Set the family ID as value
                                            child: Text(family[
                                                'name']), // Display family name
                                          );
                                        }).toList(),
                                        onChanged: (String? newValue) {
                                          setState(() {
                                ...
                                ElevatedButton(
                                  onPressed: () {
                                    setState(() {
                                      print(
                                          stat); // Assuming `stat` is a variable you want to debug or log
                                      // Call the `update` function for the staff data
                                      update(
                                        selectedDepartmentId!, // Make sure this is not null, handle null cases if needed
                                        selectedManagerId,
                                        selectedDate, // Ensure `selectedDate` is set
                                        selectgender, // Ensure `selectgender` is set
                                        selectmarital, // Ensure `selectmarital` is set
                                        selecteExp, // Ensure `selecteExp` is set
                                        selectejoin, // Ensure `selectejoin` is set
                                        selecteconf, // Ensure `selecteconf` is set
                                        scaffoldContext, // The BuildContext for displaying snackbars
                                      );

                                      // Call the `updateImage` function for the images if they are not null
                                      if (selectedImage != null ||
                                          selectedImage1 != null) {
                                        updateImage(
                                          selectedImage, // The first image
                                          selectedImage1, // The second image
                                          scaffoldContext, // The BuildContext for displaying snackbars
                                        );
                                      }
                                    });
