...
class CreatePerformaProduct_List extends StatefulWidget {
  ...
  @override
  State<CreatePerformaProduct_List> createState() => _CreatePerformaProduct_ListState();
}
...
void logout() async {
  ...
  Future<void> addtocart(
      BuildContext scaffoldContext, mainid, varid, sizeid, quantity) async {
    final token = await getTokenFromPrefs();
    try {
  ...
  Future<void> addtocart2(
      BuildContext scaffoldContext, mainid, varid, quantity) async {
    final token = await getTokenFromPrefs();
    try {
      final response = await http.post(Uri.parse('$api/api/cart/product/'),
          headers: {
          ...
          body: jsonEncode(
              {'product': mainid, 'variant': varid, 'quantity': quantity}));
      print("Response: ${response.body}");
      print("ressss${response.statusCode}");
  ...
  void showSizeDialog(BuildContext context, List<String> colors,
      List<Map<String, dynamic>> sizes, mainid, varid) {
    showDialog(
      context: context,
                      ...
                      Wrap(
                        ...
                        children: sizes.map((sizeMap) {
                          ...
                          return ChoiceChip(
                            ...
                            onSelected: (bool selected) {
                              setState(() {
                                if (selected) {
                                  ...
                                  selectedStock =
                                      stock; // Update stock based on selected size
                                  quantityController
                                      .clear(); // Clear quantity input when selecting a new size
                                } else {
                                  selectedSize = '';
                                  selectedSizeId = null;
                                  selectedStock = null;
                            ...
                            selectedColor:
                                const Color.fromARGB(223, 229, 230, 231),
                          );
                        }).toList(),
                      ),
                      SizedBox(height: 10),
                      if (selectedSize
                          .isNotEmpty) // Show quantity input field only if a size is selected
                        Padding(
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          onPressed: () {
                            ...
                            addtocart(context, mainid, varid, selectedSizeId,
                                quantity);
                            ...
                            trailing: ElevatedButton.icon(
                              onPressed: () {
                                if (product['is_vaiant'] == true) {
                                  print(
                                      "Product sizes data: ${product['sizes']}");
