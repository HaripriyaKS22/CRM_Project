  ...
  Future<void> getreciptReport() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        String getCustomerName(dynamic customerId) {
          if (customerId == null) return '';
          final found = customer.firstWhere(
            (c) => c['id'] == customerId,
            orElse: () => {},
          );
          return found != null && found['name'] != null
        ...
        // 1. Advance Receipts
        final advanceReceipts = parsed['advance_receipts'] as List<dynamic>;
        for (var reportData in advanceReceipts) {
          reciptList.add({
            ...
            'customer_name': getCustomerName(reportData['customer']),
        ...
        // 2. Bank Receipts
        final bankReceipts = parsed['bank_receipts'] as List<dynamic>;
        for (var reportData in bankReceipts) {
          reciptList.add({
            'type': 'Bank Receipt',
            'id': reportData['id'],
            'payment_receipt': reportData['payment_receipt'] ?? '',
        ...
        // 3. Payment Receipts
        final paymentReceipts = parsed['payment_receipts'] as List<dynamic>;
        for (var reportData in paymentReceipts) {
          reciptList.add({
            ...
            // 'bank': reportData['bank'] ?? '',
            'bank_name': reportData['bank_name'],
            'order_name': reportData['order_name']?.toString() ?? '',
            'customer': reportData['customer'] ?? '',
            'created_by': reportData['created_by'] ?? '',
            'created_by_name': reportData['created_by_name'] ?? '',
            'remark': reportData['remark'] ?? '',
